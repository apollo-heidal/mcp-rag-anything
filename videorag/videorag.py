    ...
    def insert_video(self, video_path_list=None):
        ...
        for video_path in video_path_list:
            ...
            if video_name in self.video_segments._data:
                ...
                last_result = {"video_name": video_name, "engine_doc_id": video_name}
                continue
            ...
            segment_index2name = split_result["segment_index2name"]
            segment_times_info = split_result["segment_times_info"]
            audio_probe = split_result["audio_probe"]
            audio_summary = split_result["audio_summary"]

            if audio_probe.get("has_audio") is False:
                raise VideoIngestError(
                    f"Video {os.path.basename(video_path)} has no detectable audio stream; native video ingest requires audio.",
                    stage="audio_probe",
                    details={"audio_probe": audio_probe, "audio_summary": audio_summary},
                )
            if audio_probe.get("has_audio") and audio_summary["segments_with_audio"] == 0:
                raise VideoIngestError(
                    f"Failed to extract usable segment audio from {os.path.basename(video_path)}.",
                    stage="segment_audio_extract",
                    details={"audio_probe": audio_probe, "audio_summary": audio_summary},
                )
            
            # Step2: obtain transcript with whisper
            transcripts, asr_summary = speech_to_text(
                video_name, 
                self.working_dir, 
                segment_index2name,
                self.audio_output_format
            )
            if audio_probe.get("has_audio") and asr_summary["segments_with_transcripts"] == 0:
                raise VideoIngestError(
                    f"Failed to transcribe extracted segment audio from {os.path.basename(video_path)}.",
                    stage="asr",
                    details={
            ...
            # Step3: obtain rough captions with the configured vision model
            try:
                captions = segment_caption(
                    video_name,
                    video_path,
                    segment_index2name,
                    transcripts,
                ...
                raise VideoIngestError(
                    f"Failed to caption video segments for {os.path.basename(video_path)}: {exc}",
                    stage="vision_caption",
                    details={
                        "audio_probe": audio_probe,
                        "audio_summary": audio_summary,
                        "asr_summary": asr_summary,
            ...
            # Step5: index rough segment descriptions for retrieval
            segment_feature_data = {
                f"{video_name}_{index}": {"content": segments_information[index]["content"]}
            ...
            last_result = {
                ...
                "engine_doc_id": video_name,
                "audio_probe": audio_probe,
                "audio_summary": audio_summary,
                "asr_summary": asr_summary,
                "segments_indexed": len(segment_index2name),
            }
