...
async def videorag_query(
    ...
    for s_id in caption_results:
        ...
        start_label = f"{start_time // 3600}:{(start_time % 3600) // 60}:{start_time % 60}"
        end_label = f"{end_time // 3600}:{(end_time % 3600) // 60}:{end_time % 60}"
        text_units_section_list.append([video_name, start_label, end_label, caption_results[s_id]])
        references.append(
            {
...
async def videorag_query_multiple_choice(
    ...
    while True:
        try:
            ...
            json_response["references"] = references
